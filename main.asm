...
msgTryAgain BYTE "Try Again?  1=yes, 0=no",0
invalidInput BYTE "invalid input",0
msgYouDied BYTE "you died ",0
msgPoints BYTE " point(s)",0
blank BYTE "                                     ",0

snake BYTE "X", 104 DUP("H")
...
strSpeed BYTE "Speed (1-extreme, 2-intermediate, 3-beginner): ",0
speed	DWORD 0
...
DrawCoin PROC						;procedure to draw coin
	mov eax, 0Eh					; Set color to yellow (black background, yellow foreground)
	call SetTextColor				; Set text color to yellow for the coin
	
	mov dl, xCoinPos
	mov dh, yCoinPos
	call Gotoxy						; Move to coin position
	
	mov al, "O"						; Draw 'O' instead of 'X'
	call WriteChar
	
	mov eax, 0Fh					; Reset color to white on black
...
YouDied PROC
	...
	mov dh, 12
	call Gotoxy
	mov edx, OFFSET msgYouDied	;"you died"
	call WriteString
	...
	movzx eax, score
	call WriteInt
	mov edx, OFFSET msgPoints	;display score
	call WriteString
